...
# Main Code #
#############
def get_all_glue_jobs(glue_client):
    all_jobs = []
    ...
    while True: 
        if 'NextToken' in jobs:
            jobs = glue_client.get_jobs(NextToken=jobs['NextToken'])
            all_jobs += jobs['Jobs']
        else:
            break
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    all_jobs = get_all_glue_jobs(glue_client)
    ...
    for job in all_jobs:
        print(job)
        try:
            configuration_name = job['SecurityConfiguration']
            ...
            if s3_encryption_config[next(iter(s3_encryption_config))] == 'DISABLED' or cloudwatch_encryption_config[next(iter(cloudwatch_encryption_config))] == 'DISABLED':
                evaluations.append(build_evaluation(job['Name'],
                                                    'NON_COMPLIANT',
                                                    event,
                                                    annotation="This job's security configuration is not encrypted for S3 and CloudWatch"))
            
            else:
                evaluations.append(build_evaluation(job['Name'],
                                    'COMPLIANT',
                                    event,
                                    annotation="This job's security configuration is encrypted for S3 and CloudWatch"))
            
        except KeyError: #security configuration doesn't exist
            evaluations.append(build_evaluation(job['Name'],
                                    'NON_COMPLIANT',
                                    event,
                                    annotation="This job's doesnt have a security configuration"))
