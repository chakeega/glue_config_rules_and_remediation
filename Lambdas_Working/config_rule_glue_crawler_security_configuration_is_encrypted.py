...
# Main Code #
#############
def get_all_glue_crawlers(glue_client):
    all_crawlers = []
    
    crawlers = glue_client.get_crawlers()
    all_crawlers += crawlers['Crawlers']
    
    while True: 
        if 'NextToken' in crawlers:
            crawlers = glue_client.get_crawlers(NextToken=crawlers['NextToken'])
            all_crawlers += crawlers['Crawlers']
        else:
            break
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    all_crawlers = get_all_glue_crawlers(glue_client)
    ...
    for crawler in all_crawlers:
        try:
            configuration_name = crawler['CrawlerSecurityConfiguration']
            ...
            if s3_encryption_config[next(iter(s3_encryption_config))] == 'DISABLED' or cloudwatch_encryption_config[next(iter(cloudwatch_encryption_config))] == 'DISABLED':
                evaluations.append(build_evaluation(crawler['Name'],
                                                    'NON_COMPLIANT',
                                                    event,
                                                    annotation="This crawler's security configuration is not encrypted for S3 and CloudWatch"))
            
            else:
                evaluations.append(build_evaluation(crawler['Name'],
                                    'COMPLIANT',
                                    event,
                                    annotation="This crawler's security configuration is encrypted for S3 and CloudWatch"))
            
        except KeyError: #security configuration doesn't exist
            evaluations.append(build_evaluation(crawler['Name'],
                                    'NON_COMPLIANT',
                                    event,
                                    annotation="This crawler's doesnt have a security configuration"))
